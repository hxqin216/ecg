...
for file in files:
    if file[0:5] in name_list:
        continue
    else:
        name_list.append(file[0:5])

for name in name_list:
    if name[1] not in ['1', '2', '3', '4', '5', '6', '7', '8', '9', '0']:
        continue
    record = wfdb.rdrecord(rootdir + '/' + name)

    if 'V5' in record.sig_name:
        V5.append(name)
...
segmented_data.to_csv('X_EU_ST-T_ECG_2.csv', index=False)
segmented_label.to_csv('Y_EU_ST-T_ECG_2.csv', index=False)
